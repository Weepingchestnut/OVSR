...
from models.common import generate_it, UPSCALE, PFRB, MetaUpscale, input_matrix_wpn


class UNIT(nn.Module):
    def __init__(self, kind='successor', basic_feature=64, num_frame=3, num_b=5, act=nn.LeakyReLU(0.2, True)):
        ...
        self.upscale = MetaUpscale(self.bf)
        print(kind, num_b)

    def forward(self, it, ht_past, scale, ht_now=None, ht_future=None):
        ...
        it_sr = self.upscale(ht, scale_coord_map, scale)
        re_it_sr = torch.masked_select(it_sr, mask.cuda())
...
class Net(nn.Module):
    def __init__(self, config):
        ...
        self.scale = config.model.train_scale[0]
        self.nf = config.model.num_frame
        self.kind = config.model.kind  # local or global
        self.act = nn.LeakyReLU(0.2, True)
        self.precursor = UNIT('precursor', self.bf, self.nf, self.num_pb, self.act)
        self.successor = UNIT('successor', self.bf, self.nf, self.num_sb, self.act)
    ...
    def forward(self, x, scale, start=0):
        self.scale = scale
        B, C, T, H, W = x.shape
        # B, T, C, H, W = x.shape
        """
        ...
        for idx in range(T):
            ...
            it_sr_pre, ht_past = self.precursor(it, ht_past, self.scale, None, None)        # it_sr_pre: torch.Size([16, 3, 256, 256]); ht_past: torch.Size([16, 56, 64, 64])
        ...
        for t in range(end):
            ...
            it_sr, ht_past = self.successor(it, ht_past, self.scale, pre_ht_all[t], ht_future)
